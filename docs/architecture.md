...
flowchart TB
  ...
  Dev --> WT[任务 Git worktree]
  WT --> Docker[受限 Docker 验证]
  Recheck --> BI[经营分析执行器]
  BI --> Catalog[受控指标目录]
  Catalog --> Source[(只读 SQLite / CSV)]
...
经营分析增加 data_sources、data_source_members、metrics、analysis_jobs、analysis_events。数据文件必须位于 `ESI_DATA_ROOTS` 白名单目录，不接受符号链接或隐藏路径。指标预先限定表、聚合、值列、日期列和维度；模型产生的计划必须再次通过后端指标白名单校验。
...
后续：ERP/CRM/数据仓库连接器、数据脱敏策略、跨源语义层、定时报表、可视化看板和 Office 导出；以及 OIDC/SSO、MFA、部门同步、PostgreSQL、队列与租约、多执行节点、强制双人复核、中央审计、预算配额、PR/CI 集成。当前不声称支持跨企业多租户隔离或恶意内核攻击下的完整安全边界。
